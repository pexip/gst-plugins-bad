...
gst_net_sim_drop_nsbuf (GstNetSim * netsim)
{
  ...
  GST_ERROR_OBJECT (netsim, "Dropping buf #%u (tokens)", nsbuf->seqnum);
...
#if 0
  if (sync_time + netsim->max_queue_delay * GST_MSECOND < now) {
    GST_ERROR_OBJECT (netsim, "Dropping too late buf #%u (delay)",
        nsbuf->seqnum);
